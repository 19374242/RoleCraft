...
import sys
sys.path.append("../")
from utils import decoder_for_openai, load_data
...
def api_worker(dataset, progress_bar, lock, write_fn, apikey):
    ...
    while True:
        ...
        p = read_file(p_path)
        persona_profile = read_file(persona_profile_path)
        p = p.format(character="", persona_profile=persona_profile)
        obj['instruction'] = p + obj['instruction'] 

        obj['output'] = completion
